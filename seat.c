							...
							uint32_t format, int32_t fd, uint32_t size)
{
	...
	{
		// Create an empty keymap when no keymap is provided
		struct xkb_keymap *keymap = xkb_keymap_new_from_names(state->xkb.context, NULL, XKB_KEYMAP_COMPILE_NO_FLAGS);
		assert(keymap);
